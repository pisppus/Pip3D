...
namespace pip3D
{
    ...
    inline constexpr float kRadToDeg = 57.295779513082320876f;
    inline constexpr float kTwoPi = 6.2831853071795864769f;
    inline constexpr float kPi = 3.14159265358979323846f;
    inline constexpr float DEG2RAD = kDegToRad;
    inline constexpr float RAD2DEG = kRadToDeg;
    inline constexpr float TWO_PI_256 = 40.74366543152520595683f;

    class FastMath
    {
    private:
        static const std::array<float, 256> sinTable;
        static const std::array<float, 256> cosTable;

        __attribute__((always_inline)) static inline float normalizeAngle(float angle)
        {
            angle = fmodf(angle, kTwoPi);
            return angle < 0 ? angle + kTwoPi : angle;
        }
