...
namespace pip3D
{
    namespace Debug
    {
        ...
        void Gizmos::setCategories(uint16_t mask)
        {
            g_categoriesMask = mask;
        }

        void Gizmos::enableCategories(uint16_t mask)
        {
            g_categoriesMask |= mask;
        }

        void Gizmos::disableCategories(uint16_t mask)
        {
            g_categoriesMask &= static_cast<uint16_t>(~mask);
        }

        uint16_t Gizmos::getCategories()
        {
